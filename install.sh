...
#    Hook registration (PostToolUse, UserPromptSubmit, Stop) and CLAUDE.md
#    block injection are handled by ClaudeCodeAdapter.install() called from
#    `mnemos install` in step 5 above. No additional shell-level wiring needed.
# ---------------------------------------------------------------------------
